...
- B08은 IndexedDB browser draft recovery와 remote-change conflict 격리 제공
- autosave debounce는 Korean IME와 mobile background 성능을 별도 검증한 후 결정
...
- B05 구현 기준은 [B05 Open File Flow Plan](b05-open-file-flow-plan.md)을 따릅니다.
...
- 현재 document URL은 `/files/:path`이며 route가 Markdown read와 tree selection의 Source of Truth입니다.
