...
KnowledgeOS는 앱 코드와 사용자 지식을 분리합니다. 앱은 `backend/`, `frontend/`, `ai/`에 있고, 사용자의 영속 지식 원본은 `knowledge/`에만 둡니다.
...
```text
KnowledgeOS/
├── ai/              # MCP server, PydanticAI agents, embedding jobs
...
- `ai/`: MCP, PydanticAI, prompt, agent 설계 문서를 둡니다.
...
- `.trash/`도 용량 제한과 보존 기간 정책이 필요합니다.
