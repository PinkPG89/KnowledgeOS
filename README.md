...
현재 backend는 application bootstrap, `/api/health`, canonical path 검증, 단일 활성 Vault containment, UTF-8 Markdown 읽기, 안전한 생성·수정, lazy Tree API, SQLite FTS incremental index sync와 Search API를 제공합니다. Frontend는 responsive file tree, Markdown deep link, 안전한 rendered preview와 CodeMirror 6 editor spike까지 구성됐으며 B06 iPhone/Android 실기기 검증을 기다리고 있습니다.
