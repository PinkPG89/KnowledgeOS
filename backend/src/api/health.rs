...
///
/// 이 함수는 검증 완료된 `AppState`를 주입받는 Axum의 `Router` 인스턴스를 생성해 리턴합니다.
...
/// `State(state): State<AppState>`는 Axum이 관리하는 읽기 전용 상태로부터
/// 설정과 검증된 `VaultRoot`를 함께 추출해 주입합니다.
///
/// * 성능 참고: `AppState`는 복제(Clone) 연산 비용이 낮은 값만 담고 있으므로
...
async fn health(State(state): State<AppState>) -> Json<HealthResponse> {
    ...
    Json(HealthResponse {
        ...
        knowledge_root: state.config.knowledge_root.display().to_string(),
    })
}
